...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        if ($notification->category == "ticket") {
            ...
            if ($notification->event == "invoice_due_reminder_before_due_date" || $notification->event == "invoice_overdue_reminder") {
                $invoice_reminder_date["due_reminder_date"] = get_my_local_time();
            }

            //to prevent multiple reminder for recurring invoice cration, we'll save the reminder date
            ...
            //save the reminder date finally
            if (count($invoice_reminder_date)) {
                $ci->Invoices_model->save($invoice_reminder_date, $notification->invoice_id);
            }
